...
# Criação de duas abas (tabs): "Dados do doente" e "Árvore de Decisão"
tabs = st.tabs(["Dados do doente", "Árvore de Decisão"])
...
with tabs[0]:
    ...
    with col1:
        ...
        unidade_idade = st.selectbox("Unidade de idade", ["Dias", "Meses", "Anos"], index=2)
    ...
    with col2:
        ...
        opc_diagnostico = diagnosticos + ["Outro (escrever)"]
        diagnostico = st.selectbox("Diagnóstico principal", options=opc_diagnostico)
        if diagnostico == "Outro (escrever)":
            diagnostico = st.text_input("Especifique o diagnóstico")
    ...
    if st.button("Submeter Dados do Doente"):
        ...
        st.write(f"**Transporte:** {data_transporte.strftime('%d/%m/%Y')} às {hora_transporte.strftime('%H:%M')}")


# --------------------- Aba 2: Árvore de Decisão (Cálculo de Risco) ---------------------
with tabs[1]:
    ...
    # Para simplificar, usaremos somente a pressão sistólica.
    ...
    def calcular_risco(fio2, peep, spo2, gcs, avpu, vis, arritmias, hipertensao):
        # Risco 5: Critico/contraindicado
